...
def open_file_dialog():
    ...
    for obj in space_objects:
        ...
        if hasattr(obj, 'orbit_line'):
            space.delete(obj.orbit_line)
...
def main():
    ...
    def toggle_orbits_handler():
        show = toggle_orbits()
        orbit_button['text'] = "Hide Orbits" if show else "Show Orbits"
        for body in space_objects:
            update_orbit(space, body)

    orbit_button = tkinter.Button(frame, text="Show Orbits", command=toggle_orbits_handler)
    orbit_button.pack(side=tkinter.LEFT, padx=5, pady=5)
